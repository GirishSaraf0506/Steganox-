...
class SteganoxApp(tk.Tk):
    ...
    def _build_embed_tab(self, parent):
        ...
        ttk.Button(parent, text="Browse", command=self._browse_embed_image).grid(row=0, column=2, padx=5)

        ttk.Label(parent, text="Secret Message:").grid(row=1, column=0, sticky='nw', padx=10, pady=8)
        ...
        ttk.Entry(parent, textvariable=self.embed_password_var, show='*', width=45).grid(row=2, column=1, padx=5)

        ttk.Label(parent, text="Output Path:").grid(row=3, column=0, sticky='w', padx=10, pady=8)
        self.embed_output_var = tk.StringVar()
        ttk.Entry(parent, textvariable=self.embed_output_var, width=45).grid(row=3, column=1, padx=5)
    ...
    def _build_extract_tab(self, parent):
        ...
        ttk.Entry(parent, textvariable=self.extract_image_var, width=45).grid(row=0, column=1, padx=5)
        ttk.Button(parent, text="Browse", command=self._browse_extract_image).grid(row=0, column=2, padx=5)

        ttk.Label(parent, text="Password:").grid(row=1, column=0, sticky='w', padx=10, pady=8)
        self.extract_password_var = tk.StringVar()
        ttk.Entry(parent, textvariable=self.extract_password_var, show='*', width=45).grid(row=1, column=1, padx=5)

        ttk.Button(parent, text="Extract Message", command=self._do_extract).grid(
            row=2, column=1, pady=15)

        ttk.Label(parent, text="Extracted Message:").grid(row=3, column=0, sticky='nw', padx=10, pady=8)
    ...
    def _do_embed(self):
        ...
        try:
            ...
            self.embed_status.config(text=f"✅ Saved to {os.path.basename(output_path)}", foreground="green")
